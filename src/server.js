...
function generateRandomInteger(maxValue) { return Math.floor(Math.random() * maxValue) }
function shouldSimulateFailure(failureRate = 0.08) { return Math.random() < failureRate }
function simulateNetworkLatency(minMs = 200, maxMs = 1200) { return new Promise((resolve) => setTimeout(resolve, minMs + generateRandomInteger(maxMs - minMs))) }
function generateId() {
...
export default function createMockServer({ environment = 'development' } = {}) {
  let mockServer = createServer({
    ...
    routes() {
      ...
      this.get('/candidates', async (schema, request) => {
        await simulateNetworkLatency()
        const queryParameters = { search: request.queryParams.search, stage: request.queryParams.stage || 'all', page: request.queryParams.page || 1 }
        const responseData = await storageService.getCandidates(queryParameters)
        return responseData
      })
      ...
      this.get('/candidates/:id', async (schema, request) => {
        ...
        const candidateData = await storageService.getCandidates({ page: 1, pageSize: 10000 })
        const foundCandidate = (candidateData.candidates || []).find((candidate) => String(candidate.id) === String(candidateId))
        if (!foundCandidate) return new Response(404, {}, { error: 'Candidate not found' })
      ...
      this.post('/candidates', async (schema, request) => {
        ...
        const emailAddress = String(candidateAttributes.email || '').trim().toLowerCase()
        if (emailAddress) {
          const allCandidates = await storageService.getCandidates({})
          const emailExists = (allCandidates.candidates || []).some((candidate) => String(candidate.email || '').trim().toLowerCase() === emailAddress)
          if (emailExists) return new Response(409, {}, { error: 'A candidate with this email already exists' })
        }
        const newCandidate = await storageService.addCandidate(candidateAttributes)
      ...
      this.put('/candidates/:id', async (schema, request) => {
        ...
        if (updateAttributes.email !== undefined) {
          const emailAddress = String(updateAttributes.email || '').trim().toLowerCase()
          if (emailAddress) {
            const allCandidates = await storageService.getCandidates({})
            const emailExists = (allCandidates.candidates || []).some((candidate) => String(candidate.email || '').trim().toLowerCase() === emailAddress && String(candidate.id) !== String(candidateId))
            if (emailExists) return new Response(409, {}, { error: 'A candidate with this email already exists' })
          }
        }
        const updatedCandidate = await storageService.updateCandidate(candidateId, updateAttributes)
        return { candidate: updatedCandidate }
      })

      this.del('/candidates/:id', async (schema, request) => {
        await simulateNetworkLatency()
        if (shouldSimulateFailure()) return new Response(500, {}, { error: 'Random write error' })
        const candidateId = request.params.id
      ...
      this.put('/jobs/:id', async (schema, request) => {
        ...
        const updateAttributes = JSON.parse(request.requestBody)
        const updatedJob = await storageService.updateJob(jobId, updateAttributes)
      ...
      this.post('/jobs/reorder', async (schema, request) => {
        ...
        if (shouldSimulateFailure(0.08)) return new Response(500, {}, { error: 'Intermittent reorder failure' })
      ...
      this.del('/jobs/:id', async (schema, request) => {
        ...
        existingJobs = existingJobs.filter((job) => String(job.id) !== String(jobId))
        await storageService.saveJobs(existingJobs)
        return { success: true }
      })
      ...
      this.put('/assessments/:jobId', async (schema, request) => {
        ...
        const savedAssessment = await storageService.saveAssessment(jobId, assessmentPayload)
      ...
      this.post('/assessments/:jobId/submit', async (schema, request) => {
        ...
        const submissionResult = await storageService.submitAssessment(jobId, submissionPayload)
      ...
      this.get('/candidates/:id/assignments', async (schema, request) => {
        await simulateNetworkLatency()
        const candidateId = request.params.id
        const assignmentsList = await storageService.getAssignments(candidateId)
      ...
      this.post('/candidates/:id/assign', async (schema, request) => {
        ...
        const updatedAssignments = await storageService.assignAssessment(candidateId, jobId)
      ...
      this.post('/auth/invite', async (schema, request) => {
        ...
        const authCredentials = await storageService.createCandidateAuth(candidateId, email, temporaryPassword)
        // simulate email outbox
        await storageService.addOutboxMessage({ to: email, subject: 'Your TalentFlow access', body: `Hello, your temporary password is: ${temporaryPassword}` })
        return { auth: { email: authCredentials.email }, password: temporaryPassword }
      })
      this.post('/auth/login', async (schema, request) => {
        ...
        if (!normalizedEmail || !providedPassword) return new Response(400, {}, { error: 'Email and password are required' })
        const authCredentials = await storageService.getCandidateAuthByEmail(normalizedEmail)
        if (!authCredentials || authCredentials.password !== providedPassword) return new Response(401, {}, { error: 'Invalid credentials' })
        // Ensure the candidate still exists
        const allCandidates = await storageService.getCandidates({})
        const candidateExists = (allCandidates.candidates || []).some((candidate) => String(candidate.id) === String(authCredentials.candidateId))
        if (!candidateExists) return new Response(401, {}, { error: 'Invalid credentials' })
        return { session: { candidateId: authCredentials.candidateId, email: authCredentials.email } }
      })
      ...
      this.post('/dev/reseed', async () => {
        ...
        await storageService.seedIfEmpty({ jobs: generatedJobs, candidates: generatedCandidates, assessments: generatedAssessments })
  ...
  (async () => {
    const generateJobData = () => {
      const availableJobTitles = ["software engineer 1", "software engineer 2", "software engineer 3", "senior software engineer", "technical lead", "ML Engineer", "DevOPS engineer"]
      const jobDataList = []
      for (let jobIndex = 1; jobIndex <= 25; jobIndex++) {
        const selectedTitle = availableJobTitles[Math.floor(Math.random() * availableJobTitles.length)]
        jobDataList.push({ id: `job-${jobIndex}`, title: selectedTitle, slug: `job-${jobIndex}`, status: 'open', tags: ['engineering', jobIndex % 2 ? 'frontend' : 'backend'], order: jobIndex, archived: false })
      }
      return jobDataList
    }
    const candidateWorkflowStages = ['Applied', 'Phone Screen', 'Onsite', 'Offer', 'Hired', 'Rejected']
    const generateCandidateData = (jobsList) => {
      const availableCandidateNames = ["Gaurav", "Arjun", "Abhinav", "Harsh", "Dhruv", "Giriraj", "Ojas", "Abhishek", "Nitin", "Armaan", "Siddharth", "Nidhan", "Archit"]
      ...
      for (let candidateIndex = 1; candidateIndex <= 900; candidateIndex++) {
        ...
        const currentStage = candidateWorkflowStages[Math.floor(Math.random() * candidateWorkflowStages.length)]
        const selectedName = availableCandidateNames[Math.floor(Math.random() * availableCandidateNames.length)]
        const normalizedEmailName = selectedName.toLowerCase()
        candidateDataList.push({ id: `cand-${candidateIndex}`, name: selectedName, email: `${normalizedEmailName}${candidateIndex}@example.com`, stage: currentStage, jobId: assignedJob.id })
      }
    ...
    const generateAssessmentData = (jobsList) => {
          ...
          sections: [
            {
              ...
              questions: [
                { type: 'short-text', label: 'Primary programming language', required: true, validation: { maxLength: 50 } },
                { type: 'long-text', label: 'Describe your most challenging project', required: true, validation: { maxLength: 500 } },
                { type: 'numeric', label: 'Years of coding experience', required: true, validation: { min: 0, max: 30 } },
                {
                  type: 'single-choice',
            ...
            },
            {
              ...
              questions: [
                ...
                { type: 'long-text', label: 'Explain the concept of inheritance in OOP', required: false, validation: { maxLength: 300 } },
              ],
            },
          ],
        },
          ...
          sections: [
            {
              ...
              questions: [
                { type: 'short-text', label: 'Largest team size you\'ve managed', required: true, validation: { maxLength: 20 } },
                { type: 'long-text', label: 'Describe a difficult team situation you resolved', required: true, validation: { maxLength: 600 } },
                { type: 'numeric', label: 'Years in leadership roles', required: true, validation: { min: 0, max: 25 } },
                {
                  type: 'single-choice',
                  ...
                  options: [
                    ...
                    { label: 'Servant Leadership', value: 'Servant Leadership' },
                  ],
                },
              ],
            },
            {
              ...
              questions: [
                ...
                { type: 'long-text', label: 'How do you handle conflicting priorities?', required: false, validation: { maxLength: 400 } },
              ],
            },
          ],
        },
          ...
          sections: [
            {
              ...
              questions: [
                { type: 'short-text', label: 'Favorite ML algorithm and why', required: true, validation: { maxLength: 100 } },
                { type: 'long-text', label: 'Explain overfitting and how to prevent it', required: true, validation: { maxLength: 400 } },
                { type: 'numeric', label: 'Years of ML experience', required: true, validation: { min: 0, max: 20 } },
                {
                  type: 'single-choice',
            ...
            },
            {
              ...
              questions: [
                ...
                { type: 'long-text', label: 'Describe your model deployment experience', required: false, validation: { maxLength: 350 } },
              ],
            },
          ],
        },
          ...
          sections: [
            {
              ...
              questions: [
                { type: 'short-text', label: 'Preferred cloud platform', required: true, validation: { maxLength: 50 } },
                { type: 'long-text', label: 'Describe a complex deployment you automated', required: true, validation: { maxLength: 500 } },
                { type: 'numeric', label: 'Years of DevOps experience', required: true, validation: { min: 0, max: 20 } },
                {
                  type: 'single-choice',
            ...
            },
            {
              ...
              questions: [
                ...
                { type: 'long-text', label: 'How do you ensure security in CI/CD pipelines?', required: false, validation: { maxLength: 400 } },
              ],
            },
          ],
        },
      ...
      for (let jobIndex = 0; jobIndex < Math.min(jobsList.length, 25); jobIndex++) {
        const currentJob = jobsList[jobIndex]
        let selectedTemplateIndex = 0

        // Choose assessment based on job title
        if (currentJob.title.toLowerCase().includes('senior') || currentJob.title.toLowerCase().includes('lead')) {
          selectedTemplateIndex = 1 // Leadership assessment
        } else if (currentJob.title.toLowerCase().includes('ml') || currentJob.title.toLowerCase().includes('machine')) {
        ...
        const selectedTemplate = predefinedAssessmentTemplates[selectedTemplateIndex]

        // Build assessment with unique IDs for this job
        const processedSections = selectedTemplate.sections.map((sectionTemplate) => ({
          id: generateId(),
          ...
          questions: sectionTemplate.questions.map((questionTemplate) => {
            ...
            if (questionTemplate.options) {
              processedQuestion.options = questionTemplate.options.map((optionTemplate) => ({
                id: generateId(),
            ...
            if (questionTemplate.showIf) processedQuestion.showIf = questionTemplate.showIf
